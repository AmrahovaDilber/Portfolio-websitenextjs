  ...
  return (
    <div className="w-full max-w-full sm:max-w-xl mt-20 sm:mt-44 mx-auto px-4 sm:px-0">
      {connection.map((item) => (
        <div
          key={item.id}
          className="flex space-x-4 sm:space-x-5 text-white mb-10 items-center"
        >
          ...
          <div className="flex flex-col">
            <p className="mb-1 sm:mb-2 text-[15px] sm:text-[17px]">
              {item.name}
            </p>
            <Link href={item.href}>
              <a className="text-lg sm:text-xl font-semibold">{item.infor}</a> {/* Added <a> tag */}
            </Link>
          </div>
        </div>
      ))}
